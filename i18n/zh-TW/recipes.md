...
| 階段 | 通道 | 從 → 到 | 時間 |
|---|---|---|---|
| wipe | 色塊寬度 | `0% → 100%`，從左邊起 | `dur 0.42  curve out` |
| hold | — | 實心強調色，沒有文字 | `0.13s` —— 一個節拍，不是停頓 |
| settle | 色塊 | 強調色 → `20–30%` 的強調色 —— 仍然明顯是那個強調色，不是灰色 | `dur 0.45  curve inout` |
| settle | 文字 | 透明 → 墨色 | 在整體大約 88% 的地方結束 |
