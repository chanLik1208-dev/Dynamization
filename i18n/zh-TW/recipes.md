...
一塊實心的強調色橫向掃過片語，停住，然後落成一層留下來的淡色底。掃過的期間文字並不在那裡 ——
它們是跟著淡入一起到場的。

| 階段 | 通道 | 從 → 到 | 時間 |
|---|---|---|---|
| wipe | 色塊寬度 | `0% → 100%`，從左邊起 | `dur 0.36  curve out` |
| hold | — | 實心強調色，沒有文字 | `0.08s` —— 一個節拍，不是停頓 |
| settle | 色塊 | 強調色 → `20–30%` 的強調色 —— 仍然明顯是那個強調色，不是灰色 | `dur 0.56  curve inout` |
| settle | 文字 | 透明 → 墨色 | `dur 0.48`，在整體大約 92% 的地方結束 |

**談總長。** 一秒超過了 `SKILL.md` §3.4 裡 `0.4–0.8s` 的頁面／敘事分級，而這是刻意的，不是疏忽：
片語到場就是這整件事的回報，趕它等於浪費掉先前那一筆替它鋪陳的 wipe。那個分級的上限之所以存在，
是為了不讓人乾等 —— 而這支動畫只跑一次、不擋住任何東西，而且在讀者讀完它周圍那句話之前就結束了。
**如果你需要把預算要回來，從 wipe 和 hold 那裡拿，不要從揭露那裡拿。** 這份配方就是照這個方向重新
調過的，而揭露是最不該被縮短的那一段。

靜止狀態 —— 淡色底加上強調過的文字 —— 就是**基礎**樣式。動畫是加上去的，所以一個沒有腳本、沒有
`IntersectionObserver`，或開了減弱動態的讀者仍然拿得到這份強調，只少了那一筆掃過。

只觸發一次，依 `recipes.md` §3。整頁最多四到五個片語：這個動畫不帶任何資訊，它只負責引導注意力，
而處處都在引導注意力，等於哪裡都沒引導。

**注意：** 四件事，而且每一件都咬過這包文件自己的參考實作。

- **絕對不要動畫字重。** 字重一變就改變前進寬度，讓整行重排。強調用的字重靜態設定一次就好，只動畫
  那個色塊。
- **實心色塊會蓋掉墨色文字。** 要嘛把文字挖成頁面的背景色 token —— 它在淺色主題是淺的、在深色主題
  是接近深的，所以同一個 token 兩邊都對 —— 要嘛就照上面的規格，在實心那段乾脆不顯示文字。不可以做
  的是讓深色文字就這樣坐在深色色塊上，就算只有 200ms 也不行。
- **如果靜止狀態會藏起文字，那一個永遠等不到動畫的片語，就等於整句話裡缺了一塊。** 加保底：如果
  observer 幾秒內都沒觸發，而片語就在畫面上，那就直接跑 → `errata.md` §A7。
- **在一個會淡入的區塊裡的片語，必須等它** → `errata.md` §A6。
- **每一段都要給緩動曲線，settle 也一樣。** 多階段的動畫通常被寫成一整條簡寫、只帶一個緩動函式，
  而如果那個函式是 `linear`，除了被關鍵影格蓋掉的地方以外，整支動畫都在等速地爬。wipe 是大家記得
  要加緩動的那一段；settle 是被忘掉的那一段，而一個硬生生停死的 settle，就是「這個淡入看起來怪
  怪的」真正的意思。緩動曲線要分段設定，設在關鍵影格裡。
- **hold 是一個節拍，不是停頓，而 settle 必須從它裡面緩緩離開。** 兩種錯誤會招來同一句抱怨 ——
  *它卡住，然後就不見了*。一個長到完全靜止的 hold（超過 `0.15s` 太多），就不再讀起來像強調，而
  開始讀起來像卡頓。而 settle 上的 `curve out` 在這裡是錯的，即使它幾乎在其他所有地方都是對的：
  easeOut 把變化都堆在前段，所以顏色在這一段的前五分之一就倒完了，色塊看起來像是消失，而不是
  安定下來。要用 `curve inout` —— 它會輕輕地離開 hold，再減速進入靜止。
- **讓文字在色塊跑完之前就落定。** 狀態的訊號，到得比承載它的表面更快（`contrast.md` §5）：文字
  在動畫大約 85% 的地方就結束，色塊則繼續緩動到 100%。兩邊用同樣的速率交叉淡化，讀起來就是一團糊。
